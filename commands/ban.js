    ...
    const banmessage = new Discord.MessageEmbed()
      ...
      .addField(
        ...
        "You can no longer access " +
          message.guild.name +
          " unless your ban is revoked."
      )
      .addField(
        "How do I appeal?",
        "Contact the Server Owner or a moderator that issued you the ban."
      )
      .setTimestamp();
    try {
      user.send({ embeds: [banmessage] }).catch((error) => {
        if (error.code === Discord.Constants.APIErrors.CANNOT_MESSAGE_USER) {
          return message.reply(
            "I couldn't message the user, but they were banned successfully!"
          );
        }
      });
