            ...
            case "search":
                ...
                async function sendEmbed() {
                    ...
                    yt_info.forEach(track => {
                        searchembed.addFields({ name: track.url, value: track.title, inline: false })
                    })
                    searchembed.setTitle("Please select a track:")
                    searchembed.setColor("#0099ff");
                    var embedmessage = await message.channel.send({ embeds: [searchembed] });
                    ...
                    var choice = 0;
                    embedmessage.awaitReactions({ filter, maxUsers: 2 })
                    .then(collected => collected.forEach(emoji => {
